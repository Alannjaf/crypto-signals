...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!q.success) return NextResponse.json(toErrorResponse("BAD_REQUEST", "Invalid query", q.error.flatten()), { status: 400 });
    const interval = q.data.interval;

    // Get top coins (server-side) to scan
    const top = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ?? ''}/api/top-cryptos`, { next: { revalidate: 300 } }).then(r => r.json() as Promise<{ coins: Array<{ binanceSymbol: string }> }> ).catch(() => ({ coins: [] as Array<{ binanceSymbol: string }> }));
    const coins: string[] = (top.coins ?? []).map((c) => c.binanceSymbol).slice(0, 100);
    if (coins.length === 0) return NextResponse.json(toErrorResponse("NO_COINS", "No coins available to scan"), { status: 502 });

    const results: Array<{ symbol: string; interval: string; strength: number; direction: 'long'|'short'|'neutral'; score: number }> = [];
    ...
    const topShorts = results.filter(r => r.direction === 'short').slice(0, 10);
    return NextResponse.json({ interval, scanned: results.length, topLongs, topShorts });
  } catch (err) {
